...
//@Component
public class WebDesignVoteTwoStream implements WebDesignVoteStream {
    ...
    private static final String OUTPUT_TOPIC = "t-commodity-web-vote-two-result";
    @Override
    @Autowired
    public void kStreamWebDesignVote(StreamsBuilder builder, ObjectMapper mapper) {
        ...
        joinTable.groupBy(
                ...
                .count().toStream()
                .print(Printed.<String, Long>toSysOut().withLabel("Vote two - color"));

        joinTable.groupBy(
                        (username, votedDesign) -> KeyValue.pair(votedDesign.getLayout(), votedDesign.getLayout()))
                .count().toStream()
                .print(Printed.<String, Long>toSysOut().withLabel("Vote two - layout"));
    }
