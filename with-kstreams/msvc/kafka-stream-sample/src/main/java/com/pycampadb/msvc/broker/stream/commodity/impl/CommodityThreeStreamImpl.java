...
//@Component
public class CommodityThreeStreamImpl implements CommodityStream {
    @Override
    @Autowired
        ...
        maskedCreditCardStream.mapValues(CommodityStreamUtil::convertToPatternMessage)
                ...
                .branch(CommodityStreamUtil.isPlasticItem(),
                        Branched.<String, OrderPatternMessage>withConsumer(ks ->
                                        ks.to("t-commodity-pattern-three-plastic", Produced.with(Serdes.String(), orderPatternSerde)
                                )
                        )
                ).defaultBranch(Branched.<String, OrderPatternMessage>withConsumer(
                        ks ->
                                ks.to("t-commodity-pattern-three-notplastic", Produced.with(Serdes.String(), orderPatternSerde))
                ));
