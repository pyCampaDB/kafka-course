...
public class LoanUdtf {
    public static final String LOAN_SUBMISSION_SCHEMA_DESCRIPTOR =
            "STRUCT<" +
            ...
            ">";
    public static final String LOAN_MONTHLY_INSTALLMENT_SCHEMA_DESCRIPTOR =
            "STRUCT<" +
            "`installmentAmount` DOUBLE, " +
            "`installmentDueDate` VARCHAR" +
            ">";

    private static final Schema LOAN_MONTHLY_INSTALLMENT_SCHEMA = SchemaBuilder
            .struct()
            .field("installmentAmount", Schema.FLOAT64_SCHEMA)
            .field("installmentDueDate", Schema.STRING_SCHEMA);

    @Udtf(
            description = "Calculate loan installment schedule, comprise of installment amount and due date",
            schema = LOAN_MONTHLY_INSTALLMENT_SCHEMA_DESCRIPTOR
    )
    public List<Struct> calculateLoanSchedule(
        ...
        var monthlyInstallmentAmount = LoanCalculator.calculateMonthlyInstallment(
                principalLoanAmount,
                annualInterestRate,
                loanPeriodMonth
        );

        IntStream.rangeClosed(1, loanPeriodMonth).forEachOrdered(
                i -> {
